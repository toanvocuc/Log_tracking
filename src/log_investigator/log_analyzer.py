...
def format_issue(severity: str, message: str) -> str:
    ...
    for line_number, line in enumerate(lines, start=1):
        ...
        if parsed is None:
            if fail_on_invalid_format:
                issues.append(
                    format_issue(
                        "ERROR", "Line {}: Invalid log format -> {}".format(line_number, line)
                    )
                )
            continue
        ...
        if fail_on_invalid_ip and not is_valid_ip(parsed["IP"]):
            issues.append(
                format_issue(
                    "ERROR", "Line {}: Invalid IP address -> {}".format(line_number, parsed["IP"])
                )
            )
